...
export default function MoveDialog({ isOpen, onClose, onConfirm, itemToMove, itemType, excludeIds = [] }: MoveDialogProps) {
  ...
  const fetchFolders = useCallback(async (parentId: string | null) => {
    ...
    try {
      ...
      setFolders(data.folders.filter((f) => !allExcludeIds.has(f.id)));
      setBreadcrumbs(parentId ? await fetchBreadcrumbs(parentId) : []);
    } catch (error) {
      console.error(error);
    } finally {
      setIsLoading(false);
  ...
  useEffect(() => {
    if (isOpen) setCurrentFolderId(null);
  }, [isOpen]);

  // Cập nhật khi currentFolderId thay đổi trong dialog
  useEffect(() => {
    if (isOpen) fetchFolders(currentFolderId);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden flex flex-col h-[60vh]">
        ...
        <div className="bg-gray-50 p-3 border-b flex flex-wrap items-center gap-1 text-sm text-gray-600">
          <button onClick={() => setCurrentFolderId(null)} className="hover:text-blue-600 flex items-center transition-colors">
            <Home size={16} />
          </button>
          {breadcrumbs.map((bc) => (
        ...
        <div className="p-4 border-t border-gray-200 bg-gray-50 flex justify-end gap-2">
          <Button variant="outline" onClick={onClose}>{t('move.cancel')}</Button>
          <Button onClick={handleConfirm} loading={isSubmitting}>{t('move.moveHere')}</Button>
        </div>
      </div>
    </div>
  );
